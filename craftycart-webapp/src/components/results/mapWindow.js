...
export default function MapWindow({storeList, loading }) {    
    ...
    const [hoveredMarker, setHoveredMarker] = useState(null); // Added hoveredMarker state
    ...
    const markerIconStyle = {
        ...
        fillColor: "#0DA74B", // Your desired color here
        fillOpacity: 1,
        scale: 0.055,
        strokeWeight: 2.5,
        strokeColor: "",
        // Center the marker icon on its position
        anchor: { x:250, y:700 },
    };
    ...
    let currentPositionIcon = {
        ...
        fillColor: '#5ECC5A', // Red color
        fillOpacity: 1,
        scale: 0.4, // Adjust the size of the circle
        strokeColor: '#eafaef', // White color for the stroke
        strokeWeight: 2, // Adjust the stroke weight
        anchor: null
    };
    ...
    const onMarkerMouseOut = () => {
        // Delay execution of onMarkerMouseOut by 3 seconds
        setTimeout(() => {
                        ...
                        >
                            ...
                            {markers.map((marker, index) => (
                                <Marker
                                    ...
                                    title={marker.title}
                                    // Set green color for the first marker
                                    icon={markerIconStyle}
                                    draggable={false}
                                    // label={marker.title}
                                    // ENABLE WHEN MARKER CLICKED IT WILL OPEN A GOOGLE MAPS LOCATION GIVEN LAT AND LNG 
                                    onClick={() => handleMarkerClick(marker.position)} // Open Google Maps on marker click
